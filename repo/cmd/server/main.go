...
func runServe(cfg *config.Config) {
	...
	orderService := services.NewOrderService(orderRepo, productRepo, addressRepo, cartRepo, userRepo, auditRepo, configRepo)
	...
	postService := services.NewPostService(postRepo, userRepo, configRepo)
	...
	// Auth endpoints (public — no auth or canary middleware)
	auth := api.Group("/auth")
	auth.POST("/login", authHandler.Login)

	// Authenticated API routes: auth first, then canary (canary needs user_id from auth)
	authed := api.Group("")
	authed.Use(middleware.AuthRequired(cfg.JWTSecret))
	authed.Use(middleware.CanaryMiddleware(configRepo, userRepo))

	// Auth endpoints (authenticated)
	authProtected := authed.Group("/auth")
	...
	authed.GET("/catalog", catalogHandler.Query)
	...
	// Payment callback (unauthenticated — comes from payment provider)
	api.POST("/payments/callback", paymentHandler.Callback)

	// Payment simulate (staff/admin)
	...
	staffShipping := authed.Group("/staff/shipping")
	...
	authed.GET("/waitlist/position", waitlistHandler.GetPosition)
	...
	authed.POST("/import", middleware.RequireRole("staff", "admin"), ieHandler.Import)
	authed.GET("/export", middleware.RequireRole("staff", "admin"), ieHandler.Export)
	authed.GET("/jobs/:id", ieHandler.GetJob)

	// KPI endpoints (admin only)
	kpi := authed.Group("/kpi")
