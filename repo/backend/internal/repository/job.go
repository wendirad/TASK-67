...
// PickPendingJobs picks up to N pending jobs for processing using SKIP LOCKED.
func (r *JobRepository) PickPendingJobs(limit int) ([]models.Job, error) {
	rows, err := r.db.Query(`
		SELECT id, type, status, payload, result, attempts, max_attempts,
		       scheduled_at, started_at, completed_at, created_at
		FROM jobs
	...
	`, limit)
	if err != nil {
		return nil, fmt.Errorf("pick jobs: %w", err)
	}
	defer rows.Close()
...
func (r *JobRepository) StartJob(jobID string) error {
	_, err := r.db.Exec(`
